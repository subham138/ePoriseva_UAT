        ...
        return new Promise((resolve, reject) => {
            ...
            request(pay_req, function (error, response) {
                ...
                xml2js.parseString(xml, (err, result) => {
                    ...
                    const data = JSON.stringify(result);
                    // console.log(data);
                    console.log({ data, req: pay_req.body });
...
const fetch_bill = async (req, res, next) => {
	...
	if(bill_details.suc != 'SUCX001'){
		...
		//res.send('<script>window.onload = function() { history.go(-1);};</script>');
		//res.render('pay/get_bill', { bill_dtls: bill_details.bill_data, ref_id: bill_details.ref_id, mode, blr_name: req.body.blr_name, BlrId: req.body.BlrId, PhNo: req.body.Field1, script: 'history.go(-1)' });
	}else{
    ...
    //res.render('pay/get_bill', { bill_dtls: bill_details.bill_data, ref_id: bill_details.ref_id, mode, blr_name: req.body.blr_name, BlrId: req.body.BlrId, PhNo: req.body.Field1 });
	res.send({ bill_dtls: bill_details.bill_data, ref_id: bill_details.ref_id, mode, blr_name: req.body.blr_name, BlrId: req.body.BlrId, PhNo: req.body.Field1, script: 'history.go(-1)' })
}
...
const pay_bill = async (req, res, next) => {
	...
	// OPEN THE CODE FOR ACTIVE PAY || START ->->->-> //
...
// }
// //res.send({body: req.body, req_body, body_str});
// // // console.log(req_body);
// const RefId = String(String(req.body.RefId).replace('"', '')).replace('"', '');
// const pay_details = await bill_pay(agent_ref_id, RefId, req.body.BlrId, req.body.PayMode, req.body.TotalAmt, type, req_body);
// res.send({ pay_details });
